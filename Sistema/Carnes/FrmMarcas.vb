Public Class FrmMarcas
    ...
    Private Sub BuscaRegistro(Codigo As String)
        Try
            Dim CMarcas As New ClsMarcas
            Dim DT As New DataTable
            DT = CMarcas.BuscaMarcas(Codigo)
            If DT.Rows.Count > 0 Then
                TxtCodigo.Text = DT.Rows(0).Item("MARCOD").ToString()
                txtDescripcion.Text = DT.Rows(0).Item("MARDES").ToString()
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End Try
    ...
    Private Sub FrmMarcas_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            Me.Close()
        End If
    End Sub

    Private Sub FrmMarcas_KeyPress(sender As Object, e As KeyPressEventArgs) Handles MyBase.KeyPress
...
ErrLinea:
    ...
    Private Sub BtnGrabar_Click(sender As Object, e As EventArgs) Handles BtnGrabar.Click
        ...
        Try
            If TxtCodigo.Text.Trim = "" Then
                Retorno = CMarcas.InsertaMarcas(txtDescripcion.Text.Trim, USUCOD)
                If Val(Retorno) > 0 Then
                    MessageBox.Show("Registro Agregado", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Call BtnNuevo_Click(sender, e)
                End If
            Else
                Retorno = CMarcas.ActualizaMarcas(TxtCodigo.Text, txtDescripcion.Text.Trim, USUCOD)
                If Val(Retorno) > 0 Then
    ...
    Private Sub BtnEliminar_Click(sender As Object, e As EventArgs) Handles BtnEliminar.Click
        ...
        Dim CMarcas As New ClsMarcas
        Try
            Retorno = CMarcas.EliminaMarcas(TxtCodigo.Text)
            If Val(Retorno) = 0 Then
